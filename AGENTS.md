...
| 26 | Accounting Foundation & Opening Balance | 🔧 In Progress | `prompt-phase-26-accounting-foundation-opening-balance.md` | **Slice A (Jurnal) + B (Saldo Awal) + C (Period Lock) DONE.** A: A13-047/049..058/094..100 + A13-096. B: A13-085..093. C: A13-101..106 (locked_until canonical, unlock confirm+reason, error state, preload+range, konteks fiscal year+periode bulanan, a11y). Gate: build/lint, route-mock 25/25, backend Journal 12/12, live 4/4. Sisa: Slice D (Tahun Fiskal A13-107..115). A13-048 di-defer ke Reports. Lihat `tracking/phase-26-progress-report.md`. |
