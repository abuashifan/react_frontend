...
| 26 | Accounting Foundation & Opening Balance | 🔧 In Progress | `prompt-phase-26-accounting-foundation-opening-balance.md` | **Slice A (Jurnal) DONE** — A13-047/049..058/094..100 + A13-096 (safe 404). Batch A1 list contract, A2 form/detail (RHF useFieldArray, dimensi, draft, workflow, revisi posted, a11y), A3 backend security + live. Gate: build/lint, route-mock 21/21, backend Journal 12/12, live 2/2. Sisa: Slice B (Saldo Awal), C (Period Lock), D (Tahun Fiskal). A13-048 di-defer ke Reports. Lihat `tracking/phase-26-progress-report.md`. |
