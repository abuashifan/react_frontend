...
interface Props { data?: CashFlowMonthData[]; isLoading?: boolean; isUnavailable?: boolean }

export function CashFlowChart({ data, isLoading, isUnavailable }: Props) {
  return (
    <div className="rounded-lg border border-[#d9e2e5] bg-white p-4">
      ...
      {isUnavailable ? (
        <div className="h-[200px]">
          <EmptyState title="Grafik belum tersedia" description="Endpoint arus kas dashboard belum tersedia." />
        </div>
      ) : isLoading ? (
