          ...
          <>
            {allRows.length > 0 && (
              <div className="flex justify-end">
                <ReportExportButton
                  variant="outline"
                  filename={`penjualan-per-pelanggan-${activeParams?.start_date ?? ''}-${activeParams?.end_date ?? ''}`}
                  sheetName="Penjualan per Pelanggan"
                  headers={['Pelanggan', 'Jml Faktur', 'Subtotal', 'Pajak', 'Total']}
                  rows={() => allRows.map((r) => [r.customer_name, toExcelNumber(r.invoice_count), toExcelNumber(r.subtotal), toExcelNumber(r.tax), toExcelNumber(r.total)])}
