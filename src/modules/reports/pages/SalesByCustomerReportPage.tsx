...
export default function SalesByCustomerReportPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['reports', 'sales-by-customer', activeParams],
    queryFn: () => reportsApi.salesByCustomer(activeParams!),
  ...
  return (
    <WorkspaceLayout
      ...
      breadcrumb={[{ label: 'Laporan', path: '/reports' }, { label: 'Penjualan', path: '/reports/sales' }, { label: 'Per Pelanggan' }]}
    >
            ...
            {allRows.length > 0 && (
              <div className="flex justify-end">
                <Button
                  ...
                  onClick={() =>
                    exportCsv(
                      `penjualan-per-pelanggan-${activeParams?.start_date ?? ''}-${activeParams?.end_date ?? ''}.csv`,
                      ['Pelanggan', 'Jml Faktur', 'Subtotal', 'Pajak', 'Total'],
                      allRows.map((r) => [r.customer_name, r.invoice_count, r.subtotal, r.tax, r.total]),
                    )
                  }
                >
                  Export CSV
              ...
              <table className="w-full text-[12px]">
                <thead className="bg-[#f8fafc]">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-right text-[11px] font-semibold uppercase tracking-wide text-[#64748b]">Jml Faktur</th>
                ...
                <tbody className="divide-y divide-[#f1f5f9]">
                  {pagedRows.map((row) => (
                    <tr key={row.customer_id} className="hover:bg-[#f8fafc]">
                      ...
                      <td className="px-3 py-1.5 text-right tabular-nums text-[#64748b]">{row.invoice_count}</td>
                  ...
                  {allRows.length === 0 && (
                    <tr>
                      <td colSpan={5} className="py-8 text-center text-[#94a3b8]">Tidak ada data penjualan untuk periode ini.</td>
                    </tr>
                  )}
                </tbody>
                {allRows.length > 0 && (
                  <tfoot className="border-t-2 border-[#cbd5e1] bg-[#f1f5f9]">
                    <tr>
                      ...
                      <td className="px-3 py-2 text-right tabular-nums font-bold text-[#1e293b]">{report.totals.invoice_count}</td>
