            ...
            {allRows.length > 0 && (
              <div className="flex justify-end">
                <Button
                  ...
                  onClick={() =>
                    exportCsv(
                      `ringkasan-pembelian-${activeParams?.start_date ?? ''}-${activeParams?.end_date ?? ''}.csv`,
                      ['Periode', 'Jumlah Tagihan', 'Subtotal', 'Pajak', 'Total'],
                      allRows.map((r) => [r.period, r.bill_count, r.subtotal, r.tax, r.total]),
                    )
                  }
                >
                  Export CSV
