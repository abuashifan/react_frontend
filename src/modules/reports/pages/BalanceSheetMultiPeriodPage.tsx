    ...
    <WorkspaceLayout hideHeader>
      <div className="space-y-4">
        <div className="no-print">
          ...
          <p className="mt-2 text-[11px] text-[#94a3b8]">Neraca bersifat per-tanggal — tiap kolom memakai tanggal akhir periode sebagai tanggal acuan.</p>
        </div>
        ...
        {!isLoading && !isError && report && cols.length > 0 && (
          <>
            <ReportPrintDocument title="Neraca Multi-Periode" paramLabel={paramLabel}>
              <table className="w-full text-[12px]">
                ...
                <tfoot>
                  <tr className="report-print-avoid-break border-t-2 border-[#cbd5e1] bg-[#f1f5f9]">
                    <td className="px-2 py-1.5 text-[11px] font-bold uppercase tracking-wide text-[#334155]">Total Kewajiban + Ekuitas</td>
                    {summary.map((s, i) => (
                      <td key={i} className="px-2 py-1.5 text-right tabular-nums font-bold text-[#1e293b]">{formatCurrency(s.total_liabilities_and_equity)}</td>
                    ))}
                  </tr>
                  <tr className="report-print-avoid-break">
                    <td className="px-2 py-1 text-[11px] font-semibold uppercase tracking-wide text-[#64748b]">Seimbang?</td>
                    {summary.map((s, i) => (
                      <td key={i} className={`px-2 py-1 text-right text-[11px] font-semibold ${s.is_balanced ? 'text-green-700' : 'text-red-600'}`}>{s.is_balanced ? 'Ya' : 'Tidak'}</td>
                    ))}
                  </tr>
                </tfoot>
              </table>
