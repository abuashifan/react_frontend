...
export default function ApVendorSummaryPage() {
  ...
  return (
    <WorkspaceLayout
      title="Ringkasan Hutang Supplier"
      breadcrumb={[{ label: 'Laporan', path: '/reports' }, { label: 'Hutang', path: '/reports/ap' }, { label: 'Ringkasan Supplier' }]}
    >
            ...
            {allRows.length > 0 && (
              <div className="flex justify-end">
                <Button
                  ...
                  className="text-[12px]"
                  onClick={() => exportCsv(
                    `ringkasan-hutang-${activeParams?.as_of_date ?? today}.csv`,
                    ['Supplier', 'Debit', 'Kredit', 'Saldo Hutang', 'Deposit Belum Dialokasikan', 'Net Exposure'],
                    allRows.map((r) => [r.vendor_name, r.debit, r.credit, r.balance, r.unapplied_deposit_total, r.net_vendor_exposure]),
                  )}
                >
                  Export CSV
                </Button>
