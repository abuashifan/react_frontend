...
export default function SalesByProductReportPage() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ['reports', 'sales-by-product', activeParams],
    queryFn: () => reportsApi.salesByProduct(activeParams!),
          ...
          <>
            {allRows.length > 0 && (
              <div className="flex justify-end">
                <ReportExportButton
                  variant="outline"
                  filename={`penjualan-per-barang-${activeParams?.start_date ?? ''}-${activeParams?.end_date ?? ''}`}
                  sheetName="Penjualan per Barang"
