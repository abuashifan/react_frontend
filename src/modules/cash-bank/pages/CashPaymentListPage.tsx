...
export default function CashPaymentListPage() {
  ...
  const query = useCashPaymentList({
    page: page + 1,
    per_page: pageSize,
  ...
  return (
    <>
      ...
      >
        <Input type="search" value={search} onChange={(event) => { setSearch(event.target.value); resetSelection() }} placeholder="Cari nomor, kontak, atau catatan..." aria-label="Cari pengeluaran kas" className="mb-3 h-9 max-w-md text-[13px]" />
        {query.isError ? <QueryErrorState error={query.error} onRetry={() => void query.refetch()} title="Pengeluaran kas gagal dimuat" /> : <DataTable
          data={visibleRows}
          columns={columns}
