...
import { purchaseSourceDocumentApi } from '../services/sourceDocumentApi'
import type { PurchaseSourceDocumentType, PurchaseSourceDocumentItem, PurchaseSourceTargetType } from '../services/sourceDocumentApi'
import type { DocumentStatus } from '@/types/common.types'

interface SourceDocumentPickerProps {
  isOpen: boolean
  onClose: () => void
  onSelect: (doc: PurchaseSourceDocumentItem) => void
  targetType: PurchaseSourceTargetType
  sourceType?: PurchaseSourceDocumentType
  vendorId?: number
  title?: string
}
...
export function SourceDocumentPicker({
  ...
  const [selected, setSelected] = useState<PurchaseSourceDocumentItem | null>(null)

  const { data, isLoading } = useQuery({
    queryKey: ['purchase', 'source-documents', targetType, sourceType, vendorId, search],
    queryFn: () => purchaseSourceDocumentApi.list({ target_type: targetType, source_type: sourceType, vendor_id: vendorId, search }),
    enabled: isOpen,
  })
