...
function PurchaseRequestFormPageContent() {
  ...
  const columns: LineItemColumn<EditableLine>[] = [
    ...
    },
    {
      ...
      render: ({ item, isReadOnly, onUpdate }) => (
        <AmountInput value={item.estimated_price} onChange={(v) => onUpdate('estimated_price', v)} disabled={isReadOnly} decimals={2} ariaLabel="estimated_price" className={cn(FLUSH_INPUT_CLASS, 'text-right')} />
      ),
    },
  ]
