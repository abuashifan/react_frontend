...
export default function PeriodLockPage() {
  ...
  const handleLock = async () => {
    ...
    try {
      ...
      toast.error(getApiErrorMessage(error, 'Gagal memperbarui periode lock.'))
    }
  }

  const handleUnlock = async () => {
    if (!overrideReason.trim()) {
      setFieldErrors({ override_reason: 'Alasan override wajib diisi untuk membuka lock.' })
      toast.error('Alasan override wajib diisi untuk membuka lock.')
      return
    }
    setFieldErrors({})
    try {
      ...
      setFieldErrors(getApiValidationErrors(error))
      toast.error(getApiErrorMessage(error, 'Gagal membuka period lock.'))
    }
  }
        ...
        {can('accounting.period-locks.manage') && (
          <FormSection title="Atur Period Lock">
            <div className="flex flex-col gap-1">
              ...
              <Input type="date" value={lockUntil} onChange={(e) => setLockUntil(e.target.value)} className={cn('h-9 text-[13px]', fieldErrorClass(fieldErrors.lock_until))} />
              <FieldError message={fieldErrors.lock_until} />
            ...
            <div className="flex flex-col gap-1">
              ...
              <Input value={overrideReason} onChange={(e) => setOverrideReason(e.target.value)} placeholder="Alasan perubahan periode lock..." className={cn('h-9 text-[13px]', fieldErrorClass(fieldErrors.override_reason))} />
              <FieldError message={fieldErrors.override_reason} />
            </div>
