...
export default function ProformaListPage() {
  ...
  const columns: ColumnDef<ProformaInvoice>[] = [
    {
      ...
      cell: ({ original }) => (
        <button type="button" onClick={() => openRecordTab({ label: original.number, path: `/sales/proformas/${original.id}` })} className="font-medium text-[#5c9ead] hover:underline">
          {original.number}
        </button>
      ),
    },
  ...
  return (
    ...
    >
      <ListSearchBar value={search} onChange={setSearch} placeholder="Cari nomor proforma, customer..." className="mb-3" />
      <DataTable
        data={data?.data ?? []}
        columns={columns}
