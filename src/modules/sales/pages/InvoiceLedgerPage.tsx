...
export default function InvoiceLedgerPage() {
  ...
  const query = useInvoiceLedger(invoiceId, {
  ...
  return (
    ...
    >
      ...
      {query.isError ? (
        <QueryErrorState error={query.error} onRetry={() => void query.refetch()} title="Buku Besar Invoice gagal dimuat" />
      ) : (
      ...
      {entries.length > 0 && (
        <div className="mt-3 flex justify-end">
          <div className="rounded-lg border border-[#d9e2e5] bg-white px-4 py-2 text-[13px]">
            ...
            <span className={cn('ml-3 tabular-nums font-semibold text-[15px]', lastBalance > 0 ? 'text-[#991B1B]' : 'text-[#065F46]')}>
              {formatCurrency(Math.abs(lastBalance))}{lastBalance > 0 ? ' (Debit)' : ''}
            </span>
          </div>
        </div>
      )}
