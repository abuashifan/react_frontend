...
import { useSalesReceiptList } from '../hooks/useSalesReceiptList'
...
export default function SalesReceiptListPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<SalesReceiptStatus | undefined>()
    ...
    <WorkspaceLayout
      ...
      action={
        <PermissionGuard permission="sales.receipts.create">
          <Button className="bg-[#e39774] hover:bg-[#d4845e] h-8 px-3 text-[13px]" onClick={() => navigate('/sales/receipts/create')}>
            <Plus className="w-3.5 h-3.5 mr-1" /> Catat Penerimaan
          </Button>
        </PermissionGuard>
      }
    >
