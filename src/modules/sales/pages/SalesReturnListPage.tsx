...
export default function SalesReturnListPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<SalesReturnStatus | undefined>()
    ...
    <WorkspaceLayout
      ...
      action={
        <PermissionGuard permission="sales.returns.create">
          <Button className="bg-[#e39774] hover:bg-[#d4845e] h-8 px-3 text-[13px]" onClick={() => navigate('/sales/returns/create')}>
            <Plus className="w-3.5 h-3.5 mr-1" /> Buat Retur
          </Button>
        </PermissionGuard>
      }
    >
