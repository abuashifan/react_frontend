...
export default function SalesReturnListPage() {
  ...
  const query = useSalesReturnList({
    page: page + 1,
    per_page: perPage,
  ...
  return (
    <>
      ...
      >
        ...
        {query.isError ? (
          <QueryErrorState error={query.error} onRetry={() => void query.refetch()} title="Retur penjualan gagal dimuat" />
        ) : (
          <DataTable
            data={visibleRows}
            columns={columns}
