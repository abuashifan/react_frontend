...
export default function SalesInvoiceListPage() {
  ...
  const [filterStatus, setFilterStatus] = useState<SalesInvoiceStatus | undefined>()
      ...
      <FilterSection title="Status">
        {STATUSES.map((s) => (
          <label key={s} className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-[12px] text-[#334155] capitalize">{s.replace('_', ' ')}</span>
          </label>
        ))}
      </FilterSection>
    ...
    <WorkspaceLayout
      ...
      action={
        <PermissionGuard permission="sales.invoices.create">
          <Button className="bg-[#e39774] hover:bg-[#d4845e] h-8 px-3 text-[13px]" onClick={() => navigate('/sales/invoices/create')}>
            <Plus className="w-3.5 h-3.5 mr-1" /> Buat Invoice
          </Button>
        </PermissionGuard>
      }
    >
